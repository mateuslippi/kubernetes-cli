...
parser = argparse.ArgumentParser(
    description="Essa ferramenta lista todas as imagens de todos os pods.",
    prog="kubernetes-get-images.py"
)

parser.add_argument("-n", "--namespace", help="A namespace Kubernetes que \
        deseja filtrar os pods.")
...
def add_to_table(pod: V1Pod, images: str, table: list):
    table.append([str_to_cyan(pod.metadata.name), str_to_magenta(images)])
